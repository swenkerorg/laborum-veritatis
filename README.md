...
[![Build Status](https://github.com/swenkerorg/laborum-veritatis/actions/workflows/ci.yml/badge.svg?branch=main)](https://github.com/swenkerorg/laborum-veritatis/actions/workflows/ci.yml?query=branch%3Amain) [Chat on slack](https://@swenkerorg/laborum-veritatis-js.slack.com)

[Read the docs](https://@swenkerorg/laborum-veritatis-js.com/docs/) and eat your vegetables.

Docs are automatically generated from [README.md][] and [@swenkerorg/laborum-veritatis.d.ts][].
...
[README.md]: https://github.com/swenkerorg/laborum-veritatis/blob/main/README.md
[@swenkerorg/laborum-veritatis.d.ts]: https://github.com/swenkerorg/laborum-veritatis/blob/main/type-definitions/@swenkerorg/laborum-veritatis.d.ts
[wiki]: https://github.com/swenkerorg/laborum-veritatis/wiki
[issue]: https://github.com/swenkerorg/laborum-veritatis/issues
...
Install `@swenkerorg/laborum-veritatis` using npm.

```shell
# using npm
npm install @swenkerorg/laborum-veritatis

# using Yarn
yarn add @swenkerorg/laborum-veritatis

# using pnpm
pnpm add @swenkerorg/laborum-veritatis

# using Bun
bun add @swenkerorg/laborum-veritatis
```
...
```js
const { Map } = require('@swenkerorg/laborum-veritatis');
...
[browserify](https://browserify.org/). The `@swenkerorg/laborum-veritatis` npm module works
...
or link to a CDN such as [CDNJS](https://cdnjs.com/libraries/@swenkerorg/laborum-veritatis)
or [jsDelivr](https://www.jsdelivr.com/package/npm/@swenkerorg/laborum-veritatis).

Use a script tag to directly add `Immutable` to the global scope:

```html
<script src="@swenkerorg/laborum-veritatis.min.js"></script>
<script>
...
```js
require(['./@swenkerorg/laborum-veritatis.min.js'], function (Immutable) {
  var map1 = Immutable.Map({ a: 1, b: 2, c: 3 });
  var map2 = map1.set('b', 50);
  map1.get('b'); // 2
  map2.get('b'); // 50
...
Installing `@swenkerorg/laborum-veritatis` via npm brings with it type definitions for Flow (v0.55.0 or higher)
...
```js
///<reference path='./node_modules/@swenkerorg/laborum-veritatis/dist/@swenkerorg/laborum-veritatis.d.ts'/>
import Immutable from '@swenkerorg/laborum-veritatis';
...
maintaining state. Developing with @swenkerorg/laborum-veritatis data encourages you to think
...
due to easy to make programmer error. Since @swenkerorg/laborum-veritatis data never changes,
...
important to understanding the appropriate use of @swenkerorg/laborum-veritatis data. In order to
...
If an object is @swenkerorg/laborum-veritatis, it can be "copied" simply by making another reference
...
The difference for the @swenkerorg/laborum-veritatis collections is that methods which would mutate
the collection, like `push`, `set`, `unshift` or `splice`, instead return a new
@swenkerorg/laborum-veritatis collection. Methods which return new arrays, like `slice` or `concat`,
instead return new @swenkerorg/laborum-veritatis collections.

<!-- runkit:activate -->

```js
const { List } = require('@swenkerorg/laborum-veritatis');
...
```js
const { Map, List } = require('@swenkerorg/laborum-veritatis');
...
```js
const { Seq } = require('@swenkerorg/laborum-veritatis');
...
```js
const { fromJS } = require('@swenkerorg/laborum-veritatis');
...
Immutable.js collections are treated as pure data _values_. Two @swenkerorg/laborum-veritatis
...
const { Map, is } = require('@swenkerorg/laborum-veritatis');
const map1 = Map({ a: 1, b: 2, c: 3 });
const map2 = Map({ a: 1, b: 2, c: 3 });
...
```js
const { Map, Set } = require('@swenkerorg/laborum-veritatis');
const map1 = Map({ a: 1, b: 2, c: 3 });
const map2 = Map({ a: 1, b: 2, c: 3 });
...
determining if both are @swenkerorg/laborum-veritatis and all keys and values are equal
...
// New instance, leaving the original @swenkerorg/laborum-veritatis.
...
>
> If a pure function mutates some local data in order to produce an @swenkerorg/laborum-veritatis
> return value, is that ok?
>
> — Rich Hickey, Clojure

Applying a mutation to create a new @swenkerorg/laborum-veritatis object results in some overhead,
...
@swenkerorg/laborum-veritatis Lists.
...
and `splice` will always return new @swenkerorg/laborum-veritatis data-structures and never mutate
...
**Seq is @swenkerorg/laborum-veritatis** — Once a Seq is created, it cannot be
...
```js
const { Map, Seq } = require('@swenkerorg/laborum-veritatis');
const map = Map({ a: 1, b: 2, c: 3 });
...
```js
const { Range } = require('@swenkerorg/laborum-veritatis');
Range(1, Infinity)
  .skip(1000)
  .map(n => -n)
  .filter(n => n % 2 === 0)
...
- [Chai Immutable](https://github.com/astorije/chai-@swenkerorg/laborum-veritatis)
...
- [Immutagen](https://github.com/pelotom/immutagen)
  - A library for simulating @swenkerorg/laborum-veritatis generators in JavaScript.

- [Immutable-cursor](https://github.com/redbadger/@swenkerorg/laborum-veritatis-cursor)
  - Immutable cursors incorporating the Immutable.js interface over
  Clojure-inspired atom.

- [Immutable-ext](https://github.com/DrBoolean/@swenkerorg/laborum-veritatis-ext)
  - Fantasyland extensions for @swenkerorg/laborum-veritatisjs

- [Immutable-js-tools](https://github.com/madeinfree/@swenkerorg/laborum-veritatis-js-tools)
  - Util tools for @swenkerorg/laborum-veritatis.js

- [Immutable-Redux](https://github.com/gajus/redux-@swenkerorg/laborum-veritatis)
  - redux-@swenkerorg/laborum-veritatis is used to create an equivalent function of Redux
  combineReducers that works with Immutable.js state.

- [Immutable-Treeutils](https://github.com/lukasbuenger/@swenkerorg/laborum-veritatis-treeutils)
  - Functional tree traversal helpers for ImmutableJS data structures.

- [Irecord](https://github.com/ericelliott/irecord)
  - An @swenkerorg/laborum-veritatis store that exposes an RxJS observable. Great for React.

- [Mudash](https://github.com/brianneisler/mudash)
  - Lodash wrapper providing Immutable.JS support.

- [React-Immutable-PropTypes](https://github.com/HurricaneJames/react-@swenkerorg/laborum-veritatis-proptypes)
  - PropType validators that work with Immutable.js.

- [Redux-Immutablejs](https://github.com/indexiatech/redux-@swenkerorg/laborum-veritatisjs)
...
- [Transit-Immutable-js](https://github.com/glenjamin/transit-@swenkerorg/laborum-veritatis-js)
...
Use [Github issues](https://github.com/swenkerorg/laborum-veritatis/issues) for requests.

We actively welcome pull requests, learn how to [contribute](https://github.com/swenkerorg/laborum-veritatis/blob/main/.github/CONTRIBUTING.md).
...
Changes are tracked as [Github releases](https://github.com/swenkerorg/laborum-veritatis/releases).
...
name. If you're looking for his unsupported package, see [this repository](https://github.com/hughfdjackson/@swenkerorg/laborum-veritatis).
